...
router.get('/delito-tipos', async (req, res) => {
    try {
        ...
        const [rows] = await dbCnn.query(sql);
        res.json(rows);
...
router.get('/delito-tipos/:id', async (req, res) => {
    try {
        const [rows] = await dbCnn.query('SELECT * FROM delito_tipo WHERE id=?', [req.params.id]);
...
router.post('/delito-tipos', async (req, res) => {
    try {
        ...
        const [[row]] = await dbCnn.query('SELECT * FROM delito_tipo WHERE id=?', [r.insertId]);
...
router.put('/delito-tipos/:id', async (req, res) => {
    try {
        ...
        const [[row]] = await dbCnn.query('SELECT * FROM delito_tipo WHERE id=?', [req.params.id]);
        res.json(row);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/delito-tipos/:id', async (req, res) => {
    try {
        const [r] = await dbCnn.query('UPDATE delito_tipo SET activo=0 WHERE id=?', [req.params.id]);
        if (!r.affectedRows) return res.status(404).json({ error: 'Not found' });
        res.json({ ok: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
