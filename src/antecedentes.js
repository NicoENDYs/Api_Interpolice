...
router.get("/antecedentes/:id", async (req, res) => {
  try {
    const [[a]] = await dbCnn.query("SELECT * FROM antecedente WHERE id=?", [req.params.id]);
    if (!a) return res.status(404).json({ error: "Not found" });

    // 1) Tipos del antecedente
    const [t1] = await dbCnn.query(
      `SELECT dt.nombre
...
router.post("/antecedentes", async (req, res) => {
  const conn = await dbCnn.getConnection();
  try {
    const {
      ciudadano_codigo,
      evento_id,
    ...
    if (autoridad_usuario_id != null) {
      const [[u]] = await dbCnn.query("SELECT 1 FROM usuarios WHERE id=?", [
        autoridad_usuario_id,
      ]);
      if (!u)
        return res
    ...
    await conn.commit();
    const [[row]] = await dbCnn.query("SELECT * FROM antecedente WHERE id=?", [
      r.insertId,
    ]);
